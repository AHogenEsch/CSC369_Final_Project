...
The union of these queries produced **75,540 unique English Wikipedia article titles** (74,941 politicians and 599 political parties). Spot-checking confirmed that all major expected names were present: Barack Obama, Donald Trump, Joe Biden, Abraham Lincoln, George Washington, Nancy Pelosi, Alexandria Ocasio-Cortez, Democratic Party (United States), Republican Party (United States), and others.
...
6. Writes matched data to Parquet files using PyArrow with Snappy compression
...
Files are processed sequentially (one at a time) to keep resource usage low -- approximately one CPU core and ~100 MB of RAM regardless of input file size.

### 2.4 Exploratory Analysis of Extracted Data

After filtering file 1 as a validation run, the extracted Parquet data was analyzed to confirm it contained sufficient information for the project goals:

- **Revision frequency over time**: The `timestamp` field allows binning revisions by any time granularity. A test on the Abraham Lincoln article revealed clear temporal patterns -- heavy editing in 2005--2006, a resurgence in 2020 and 2025 -- consistent with known periods of public interest.
- **Vandalism detection signals**: Multiple independent signals were identified in the data:
  ...
  - **Minor edit flag**: 26.1% of edits are flagged minor; vandalism is almost never flagged as such.

### 2.5 Coverage Validation

After processing all 27 files, the coverage report confirmed:
...
### 3.3 Windows Console Encoding Error

**Issue**: When the exploration script attempted to print Unicode box-drawing characters (` ── `) to the Windows console, it crashed with a `UnicodeEncodeError` because the default Windows code page (cp1252) cannot encode those characters.

**Resolution**: The box-drawing characters were replaced with plain ASCII equivalents (`--`). All scripts were written to use ASCII-safe output to ensure compatibility with the Windows terminal.

### 3.4 Output Buffering in Background Execution
...
### 3.5 Compressed File I/O and Disk Space Constraints
...
### 3.6 Null and Missing Value Patterns
...
**Resolution**: These null patterns are inherent to the Wikipedia data model, not data corruption. The Parquet schema uses nullable types for these columns. Analysis code must account for these patterns -- for example, checking `contributor_ip IS NOT NULL` to identify anonymous edits rather than assuming a missing username indicates bad data.

### 3.7 Non-US Politicians in Wikidata Results
